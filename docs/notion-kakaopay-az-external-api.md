# 카카오페이 ↔ 에즈 ↔ 보맵 API 연동 명세 (외부 공유용)

> 카카오페이 보험상담 연동의 시스템 간 API 계약을 **호출 방향별**로 정리한 문서.
...
| 구간 | 망 노출 | 인증 방식 | 전송 |
|---|---|---|---|
| 카카오페이 ↔ 에즈 | 공인 인터넷 | `PARTNER_KEY` (파트너 인증 키) | TLS |
| 에즈 ↔ 보맵 | **내부 폐쇄망(사설 VPC)** | 망 분리(대부분 무인증) · 설문 콜백만 공유키 | TLS |

- **카카오페이 ↔ 에즈** — 공인 인터넷 구간. 모든 요청은 카카오페이가 발급한 `PARTNER_KEY` 헤더로 인증하고 TLS로 암호화 전송한다.
...
| 메서드 | 경로 | 용도 |
|---|---|---|
| POST | `/consultations/kakaopay` | 상담 신청 접수(트리거) |
...
> 헤더 `Authorization: PARTNER_KEY {키}`. JSON 키는 **snake_case**. 에즈가 카카오페이로 고객정보를 조회(pull)한다.
...
| 메서드 | 경로 | 용도 | 인증 |
|---|---|---|---|
| POST | `/external/consultations/kakaopay` | 상담 신청 통보 | 망 분리(무인증) |
| PUT | `/external/consultations/kakaopay` | 상담 만료/취소 통보 | 망 분리(무인증) |
| POST | `/external/additional-info/status-changes` | 설문(추가정보) 상태변경 통보 | 공유키 `x-client-id` / `x-api-key` |

**POST `/external/consultations/kakaopay` — 요청**
...
**PUT `/external/consultations/kakaopay` — 요청**

| 필드 | 타입 | 길이 | 보호 | 필수 |
|---|---|---|---|---|
| kakaoPayMemberId | Long | — | 평문(폐쇄망) | 필수 |
...
| 메서드 | 경로 | 용도 |
|---|---|---|
| GET | `/api/v1/kakaopay-members?userKey&consultationUuid[&memberId]` | 회원 조회·매핑·userKey 갱신 |
| GET | `/api/v1/kakaopay-members/consultations/{consultationUuid}` | 상담 UUID로 회원 조회 |
| GET | `/api/v1/kakaopay-members/members/{userKey}` | userKey로 회원 조회 |
| POST | `/api/v1/kakaopay-members/list` | memberId 목록으로 다건 조회 |
| GET | `/api/v1/kakaopay-members/list/name?name=` | 이름으로 검색 |
| GET | `/api/v1/kakaopay-members/list/phone?phoneNumber=` | 전화번호로 검색 |
| PUT | `/api/v1/kakaopay-members/{memberId}/restore?consultationUuid[&userKey]` | 개인정보 복원 |
...
**`POST /api/v1/kakaopay-members/list` — 요청**
...
|---|---|---|---|
| `PARTNER_KEY` | 카카오페이 ↔ 에즈 | 카카오페이 발급 | 정적 키 — 애플리케이션 레벨 만료 없음. 키 회전은 카카오페이 정책에 따름. 환경변수로 주입. |
...
- 핵심 카카오페이 **회원·상담 연동(§2-1, §2-4)은 폐쇄망 보호**로 별도 토큰을 사용하지 않는다.
...
- **전송**: 전 구간 TLS. 외부(카카오페이) 구간은 `PARTNER_KEY` 인증, 내부(에즈↔보맵) 구간은 폐쇄망 격리.
