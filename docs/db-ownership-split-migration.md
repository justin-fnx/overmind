...
### 2-6. DMS validation 은 동일-인스턴스+collation 에서 미덥지 않다
- CDC-only validation 이 정체(대부분 "Not enabled"), `chat.ban_word`는 "Table error"
  이나 `ValidationFailedRecords=0`(실데이터 정상, 대조만 실패 — 한글 varchar collation 추정).
- VALIDATOR 로그 컴포넌트 미활성 → CloudWatch에 사유 안 남음.
- **해법**: 1차 검증 = **행수 대조**(`gen_count_check.py`) + 필요 시 pt-table-checksum.
  DMS validation은 보조로만.
