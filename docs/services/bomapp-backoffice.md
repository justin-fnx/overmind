...
중요 규칙: next-backend 등 앱 소유 데이터에 write 가 필요하면 backoffice DB 직접 변경이 아니라 owner API 를 먼저 만든 뒤 proxy 한다. 조회라도 owner admin API 가 이미 제공되는 기능은 직접 DB 집계로 되돌리지 않고 BFF proxy 로 연결한다. upstream API 가 없으면 화면/로직은 stub 으로 두고 나중에 연결한다.
...
| next-backend / bomapp-api | sync REST | `BACKOFFICE_ADMIN_BASE_URL` 로 admin 기능 proxy. 지표 화면은 `GET /admin/backoffice/metrics/engagement` 를 호출해 DAU/MAU, 유입, 탈퇴, 추이 데이터를 가져온다. |
