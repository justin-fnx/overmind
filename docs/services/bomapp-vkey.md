...
| ALB 라우팅 | PROD-ALB:443 priority 10 → **TG `prod-bomapp-vkey-ip-8080` (단일 TG, BOM-83 디커미션으로 옛 8080 leg 제거)**. priority 4 (X-Canary=office 헤더 룰) 도 동일 TG — 헤더 검증 기반이라 재사용 위해 **유지**. priority 5 (사무실 source-ip 룰) 은 MR [!43](https://gitlab.bomapp.co.kr/bomapp/infra/-/merge_requests/43) 으로 **제거됨**(컷오버 완료, redundant). |
| 컨테이너 포트 | 8080 |
...
## 10. 소비자(consumer) 환경별 vkey 라우팅 (2026-06-16 확인)

**질문: "보맵 앱에서 vkey 는 항상 prod 만 바라보는가?" → 답: 아니다(설계상). 단, 두 소비자에서 비-prod 환경이 사실상 PROD vkey 로 누수된다.**

### 10.1 인프라는 3환경 격리되어 있다

infra(Terraform)는 vkey 를 dev/stg/prod 각각 별도 ECS service + TG + ALB host 룰로 **완전 격리** 제공한다:

| 환경 | 도메인 | DNS | ALB 라우팅 | 대상 |
|------|--------|-----|-----------|------|
| prod | `vkey.bomapp.co.kr` | 15.164.25.64 | prod-alb:443 rule_1(prio 10) host=`vkey.bomapp.co.kr` | TG `prod-bomapp-vkey-ip-8080` → PROD-Cluster |
| stg | `stg-vkey.bomapp.co.kr` | 15.164.25.64 (= prod-nlb, STG 가 PROD VPC/ALB 공유) | prod-alb:443 **rule_30** host=`stg-vkey.bomapp.co.kr` | TG `stg-bomapp-vkey-ip-8080` → STG-Cluster |
| dev | `dev-vkey.bomapp.co.kr` | 43.200.188.255 (dev-alb) | dev-alb:443 rule(prio 70) host=`dev-vkey.bomapp.co.kr` | TG `dev-bomapp-vkey-ip-8080` → DEV-Cluster |

> stg-vkey 는 prod 와 같은 IP(prod-nlb)로 해석되지만, **prod-alb host-header rule_30 이 별도 STG TG 로 분기**하므로 stg 트래픽은 실제 STG vkey ECS 로 간다(prod 와 격리됨).

### 10.2 소비자별 설정 — 과거 두 군데 PROD 누수 → ✅ 2026-06-16 격리 수정

> **수정 이력 (2026-06-16)**: 아래 두 누수를 모두 격리하는 변경 생성 — next-backend [PR #465](https://github.com/bomapp-inc/next-backend/pull/465) (base `dev`), next-frontend [MR !503](https://gitlab.bomapp.co.kr/bomapp/next-frontend/-/merge_requests/503) (target `main`). **머지 대기**. 아래 표는 수정 후 기준이며 비고에 과거 누수 상태를 병기.

| 소비자 | 위치 | dev | stg | prod | 비고 |
|--------|------|-----|-----|------|------|
| **next-backend / bapi** (Feign decode) | `bomapp-external/raon/.../application-external-raon-{env}.yml` (`raon.transkey.servlet-url`) | `dev-vkey.bomapp.co.kr` ✓DEV | `stg-vkey.bomapp.co.kr` ✓STG | `vkey.bomapp.co.kr` ✓PROD | PR#465 로 dev 수정(과거: dev 가 `vkey.bomapp.co.kr` PROD 직접 호출 = 누수). stg/prod 무변경. |
| **next-frontend / bomapp-web** (가상키보드 init) | `public/TouchEn/transkey_mobile/transkey.js` `switch(location.host)` | `dev-web.bomapp.co.kr`→`dev-vkey` ✓DEV | `stg-web.bomapp.co.kr`→`stg-vkey` ✓STG | `www`/`web`/`web-2z9w75bv.bomapp.co.kr` + `default`→`vkey` ✓PROD | MR!503 로 수정(과거: dev/stg case 가 `dev30/stg30 ...:8080` = **NXDOMAIN** 죽은 호스트라 현재 dev/stg 프론트는 `default`→PROD 로 누수). |

- **bapi(decode 경로)**: 서버측 Feign(서버-서버) 호출이라 CORS 무관. dev 를 `dev-vkey.bomapp.co.kr`(dev-alb prio70 → DEV vkey TG) 로 교체해 격리. Raon 라이선스가 `*.bomapp.co.kr` 와일드카드라 전 환경 동작.
- **transkey.js(가상키보드 init/getKey 경로)**: 죽은 `dev30/stg30:8080` 분기를 현행 프론트 호스트(`.env` 의 `VITE_VUE_SERVER`: dev=`dev-web`, stg=`stg-web`)로 교체 + `dev-vkey`/`stg-vkey` 라우팅. prod(`www`/`web`/`web-2z9w75bv`)+`default` 는 PROD vkey 유지(안전망). `localhost:3100` 로컬은 `dev-vkey`(브라우저 origin 이 dev-vkey CORS `*.bomapp.co.kr` 에 미매칭 시 로컬 한정 CORS 가능 — 블로커 아님, 별도 추적 후보).
- **네이티브 앱**(bomapp-aos `:mtranskey`, bomapp-ios): 서버 URL 하드코딩 없음 — 웹뷰(bomapp-web)가 로드하는 transkey.js 를 통해 서버 URL 을 받으므로 위 프론트 동작을 그대로 상속(즉 MR!503 머지 시 앱도 자동 격리).

### 10.3 종합

- 인프라는 dev/stg/prod vkey 를 처음부터 격리 제공(각 ECS+TG+ALB host 룰, `dev-vkey`/`stg-vkey` 모두 라이브). **과거 문제는 소비자 설정이 이를 따라오지 못한 것** — bapi dev=PROD 직접 호출, 프론트 dev/stg=죽은 호스트→default→PROD.
- **2026-06-16 두 소비자 모두 격리 수정 생성**(PR#465 / MR!503, 머지 대기). 머지 후 dev/stg/prod 가 각자 환경 vkey 로 정확히 라우팅된다.
- 잔여(머지 후 확인): 머지·배포 후 dev/stg 에서 가상키보드 init·decode 가 dev-vkey/stg-vkey 로 실제 라우팅되는지 런타임 검증(RequestCount).

### 10.4 legacy-backend (`bomapp_api_server`) — vkey 소비 존재하나 격리 제외 (2026-06-16)

legacy 모놀리스 `bomapp_api_server` 에도 vkey decode 소비자 `SecureServletClient` 가 있다(`@FeignClient url = ${raon.transkey.servlet_url}`, `POST /transkeyServlet/decode`, `GnnetClientTest` 에서 사용). 환경 설정(`application-{profile}.properties` 의 `raon.transkey.servlet_url`)이 매우 혼란스럽다:

| 프로파일 | `raon.transkey.servlet_url` | 평가 |
|----------|------------------------------|------|
| local | `http://localhost:8080` | 로컬 |
| dev | `https://dev30.bomapp.co.kr:8080` | ☠️ NXDOMAIN(죽은 호스트) |
| dev-int | `https://dev.bomapp.co.kr:8080` | 확인 필요 |
| stg | `http://10.1.1.20:8080` | ⚠️ **PROD IP(api-was2) 누수** |
| stg-int | `https://stg.bomapp.co.kr:8080` | 확인 필요 |
| prod | `http://10.1.1.20:8080` | 옛 PROD-BACK 공용 WAS vkey leg — **BOM-83 디커미션으로 제거됨 = 깨졌을 가능성** |
| dr | `http://10.10.90.10:8080` | DR |

→ **격리 제외 결정(2026-06-16, 사용자 판단)**: legacy `bapi` 는 next-backend `bomapp-api` 로 컷오버됨(PROD-BACK 해체 시 bapi leg 빠짐). vkey 활성 트래픽은 next-backend(PR#465) 가 처리하므로 legacy 의 위 설정은 **dead config** 로 간주하고 수정하지 않는다. 단 infra 에 `/ecs/prod-legacy-api-was` 로그그룹·`legacy-bomapp-api` ECR(legacy_migration) 흔적이 있어, **legacy api 가 어떤 형태로든 (재)가동되면 stg/prod=`10.1.1.20:8080` 의 깨진/누수 엔드포인트가 표면화될 위험**이 있다. legacy 폐기 또는 재가동 시 이 설정을 정리할 것. (CLAUDE.md: legacy 신규기능 금지 + 차세대 이관 우선이라 격리 수정도 차세대 경로 우선.)
