...
| [bomapp_my_data](./services/bomapp_my_data.md) | 백엔드 | Java 11 / Spring Boot 2.3 | **이관 중 (레거시)** | 마이데이터 동의/관리 (mydata-api 로 이관) |
...
graph LR
  ...
  subgraph Mydata[마이데이터 계층]
    ...
    legacy_mydata[bomapp_my_data 레거시]
  end
...
| 호출자 → | next-backend API 들 | mydata-agent | bomapp_my_data | 외부 파트너 |
|----------|--------------------|--------------|----------------|------------|
...
| 시스템 | 모듈 | 용도 |
|--------|------|------|
| AZ (보험사) | `external-az`, `external-az-managed` | 보험 상담, 설계사 도구 |
...
| `bomapp_my_data` | `next-backend/mydata-api` | 진행 중. PROD-BACK 컨테이너에 `bomappmydata-0.0.1-SNAPSHOT.jar` (PID 15890) 활성 |
...
- 본 디렉토리 [services/](./services/) — 각 서비스 상세
