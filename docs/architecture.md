...
| [bomapp-backoffice](./services/bomapp-backoffice.md) | 내부 백오피스 | Bun / Next.js 16 / NestJS 11 | **활발 · 배포 준비 중** | redmin 대체 내부 운영 콘솔. infra MR !54는 반영, 앱 배포 MR !7은 2026-06-23 조회 기준 open/pipeline failed |
...
| [mydata-mgmts-api](./services/mydata-mgmts-api.md) | 백엔드 | Java 21 / Spring Boot 3.4.5 | **✅ PROD 신규 ECS 컷오버 완료 (2026-06-11)** | 마이데이터 표준 동의·관리(mgmts) 수신, 규제필수 |
...
graph LR
  ...
  subgraph CDN[CloudFront + S3]
    ...
    backoffice_front[bomapp-backoffice frontend]
  end
  ...
  subgraph Backoffice[내부 백오피스]
    backoffice_bff[bomapp-backoffice Admin BFF]
  end
  ...
  Browser --> backoffice_front --> backoffice_bff
  Browser --> chat_api
  ...
  backoffice_bff -->|admin API proxy| bomapp_api
  backoffice_bff -->|admin API proxy| wings_api
  ...
  open_api --> Aurora
  wings_api --> Aurora
  backoffice_bff --> Aurora
  backoffice_bff --> Redis
...
| bomapp-backoffice | ✓ (Admin BFF proxy: bapi/wapi) | | | |
...
| `admin-api.bomapp.co.kr` | `dev-admin-api.bomapp.co.kr` | — | 4000 | **bomapp-backoffice Admin BFF** — infra MR !54 기준 신규 ECS `SVC-ECS-{ENV}-bomapp-backoffice` / TG 라우팅 리소스 반영. 앱 배포 MR !7은 별도이며 frontend 는 `/api` base 로 호출 |
...
| `dev-admin.bomapp.co.kr` | — | `admin.bomapp.co.kr` | bomapp-backoffice frontend | `dev-admin.bomapp.co.kr` / `admin.bomapp.co.kr` |
...
> **backoffice 반영 상태**: infra MR !54(BOM-209)로 `SVC-ECS-DEV-bomapp-backoffice` / `SVC-ECS-PROD-bomapp-backoffice` 리소스가 Terraform main에 반영되었다. backoffice 앱 배포 파이프라인/manifest 는 별도 GitLab MR !7 기준이며, 2026-06-23 조회 시점에는 open/pipeline failed 상태다. 위 running task 수는 2026-05-19 실측값이라 실제 service apply/deploy 후 재검증이 필요하다.
...
| 컴포넌트 | 사용 서비스 | 비고 |
|---------|------------|------|
| **Aurora MySQL (RDS)** | next-backend 전 앱, legacy-backend, mydata-mgmts-api, bomapp-backoffice | DEV/STG/PROD 환경별. Terraform 미관리 (수동). backoffice 는 Admin BFF/Prisma 가 사용 |
| **ElastiCache Redis (Serverless, TLS)** | next-backend (Redisson), chat-api, mydata-mgmts-api, bomapp-backoffice | 과거 PROD-Chat 전용(`prod-chat-redis`)은 **2026-06-11 폐기** — 공용 Redis(Serverless)로 통합. backoffice 는 express-session store 로 사용 |
...
| 서비스 | 도구 | 빌드 산출물 |
|--------|------|------------|
| next-backend | GitLab CI/CD (정본 이전 완료, GitHub Actions 과도기 잔존) | Jib 멀티아키 OCI 이미지 (amd64/arm64) → ECR |
| next-frontend | GitLab CI/CD | Vite 정적 빌드 → S3 + CloudFront 무효화 |
| bomapp-backoffice | GitLab CI/CD (MR !7 기준, 2026-06-23 조회 시 open/pipeline failed) | Backend Docker(ARM64) → ECR/ECS, Frontend static export → S3 + CloudFront |
...
CI 가 운영하는 GitLab(온프레미스 `172.16.100.244`) 이 현재 SCM 기본 정본이다. GitHub 는 `bomapp-vkey`, `mydata-agent`, 일부 레거시 리포 등 예외가 남아 있으므로 작업 시 `services.yaml`의 `repo_url`을 먼저 확인한다.
...
| `mydata-mgmts-api` | 독립 ECS 서비스 + next-backend 연계 | **2026-06-11 PROD 신규 ECS 100% 컷오버 완료**. 구 PROD-BACK 11000 jar 는 weight 0 롤백 stub 로 잔존 |
