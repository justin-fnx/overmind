...
| [bomapp-console](./services/bomapp-console.md) | 내부 운영 콘솔 | Bun / Next.js 16 / NestJS 11 | **활발 · 배포 준비 중** | redmin 대체 내부 운영 콘솔. Terraform/manifest 계약은 console 기준이나 실제 AWS service apply/deploy는 아직 미완료 |
...
graph LR
  ...
  subgraph CDN[CloudFront + S3]
    ...
    console_front[bomapp-console frontend]
  end
  ...
  subgraph Console[내부 운영 콘솔]
    console_bff[bomapp-console Admin BFF]
  end
  ...
  Browser --> console_front --> console_bff
  Browser --> chat_api
  ...
  console_bff -->|Bomapp Ops admin API proxy| bomapp_api
  console_bff -->|wings admin API proxy| wings_api
...
| bomapp-console | ✓ (Admin BFF proxy: bapi/wapi) | | | |
...
| `console-api.bomapp.co.kr` | `dev-console-api.bomapp.co.kr` | — | 4000 | **bomapp-console Admin BFF** — Terraform/manifest 계약 기준 신규 ECS `SVC-ECS-{ENV}-bomapp-console` 예정. 실제 AWS service apply/deploy 미완료이며 frontend 는 `/api` base 로 호출 |
...
| `dev-console.bomapp.co.kr` | — | `console.bomapp.co.kr` | bomapp-console frontend | `dev-console.bomapp.co.kr` / `console.bomapp.co.kr` |
...
> **bomapp-console 반영 상태**: Terraform/manifest 계약은 BOM-221 기준 `SVC-ECS-{ENV}-bomapp-console`, `dev-console*.bomapp.co.kr`, `console*.bomapp.co.kr` 로 정리되었다. 다만 실제 AWS service apply 와 앱 배포는 아직 완료되지 않았으므로 작업 전 live ECS/S3/CloudFront 상태를 재검증해야 한다.
...
| 컴포넌트 | 사용 서비스 | 비고 |
|---------|------------|------|
| **Aurora MySQL (RDS)** | next-backend 전 앱, legacy-backend, mydata-mgmts-api, bomapp-console | DEV/STG/PROD 환경별. Terraform 미관리 (수동). bomapp-console 은 Admin BFF/Prisma 가 사용 |
| **ElastiCache Redis (Serverless, TLS)** | next-backend (Redisson), chat-api, mydata-mgmts-api, bomapp-console | 과거 PROD-Chat 전용(`prod-chat-redis`)은 **2026-06-11 폐기** — 공용 Redis(Serverless)로 통합. bomapp-console 은 express-session store 로 사용 |
...
| bomapp-console | GitLab CI/CD (AWS service apply/deploy 미완료) | Backend Docker(ARM64) → ECR/ECS, Frontend static export → S3 + CloudFront |
