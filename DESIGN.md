...
- **Worker Teammates:** 특정 리포지토리에서 격리된 환경으로 실행되는 하위 에이전트. 실제 코드 수정, 테스트, MR/PR 생성을 담당.
- **MCP Integration:** Linear(이슈 트래킹)과 Git provider(GitLab/GitHub 코드 변경)를 양방향으로 동기화하여 작업의 전 과정을 추적 가능하게 함.
...
| MCP 도구 접근 (Linear, GitLab/GitHub) | 모든 Teammate이 MCP 서버 자동 로드 |
| 코드 편집/테스트/MR·PR | Claude Code의 전체 편집 능력 내장 |
...
- **Responsibilities:**
  ...
  3. 테스트/린트 실행 및 결과 보고.
  4. MR/PR 생성 및 Diff 요약을 Leader에게 메시지로 보고.
- **Execution Model:**
...
Teammate → Leader:  완료 보고 (MR/PR 링크, 변경 사항 요약, 에러 발생 시 상세)
...
| **Git provider MCP/CLI** | 브랜치 관리, 파일 읽기/쓰기, MR/PR 생성, **MR/PR 리뷰**. GitLab 정본은 `glab`, GitHub 잔존 리포는 `gh` 사용 | Leader (리뷰), Teammate (MR/PR 생성) |
...
services:
  - name: "order-service"
    repo_url: "gitlab.example.com/org/order-service"
    path: "./services/order"
  ...
  - name: "payment-service"
    repo_url: "gitlab.example.com/org/payment-service"
  ...
  - name: "user-service"
    repo_url: "gitlab.example.com/org/user-service"
    path: "./services/user"
...
- Teammate 완료 보고를 받으면 반드시 인수 조건 검증 → MR/PR 리뷰 순서를 따른다.
```
...
[5] Spawn Teammate A (Leader → user-service)
    ...
    │  MR/PR을 생성하고, 변경된 user.proto 내용을 리더에게 보고.    │
    │  Plan approval을 요구한다."                                │
    └──────────────────────────────────────────────────────────┘
...
[6] Teammate A 실행
    ...
    ├→ MR/PR 생성
    └→ Leader에게 메시지: "완료. MR/PR: [link]. 변경된 proto: ..."
...
[11] Finalize (Leader)
     ├→ 모든 Task 인수 조건 충족 + MR/PR 리뷰 완료 확인
     ├→ Linear LIN-142에 코멘트:
     │   "2개 서비스 수정 완료. 인수 조건 모두 충족.
     │    - user-service: MR !45 (ID → UUID v7) ✅ reviewed
     │    - order-service: MR !78 (FK 타입 변경) ✅ reviewed
     │    리뷰 부탁드립니다."
...
| MR/PR 리뷰에서 문제 발견 | Leader가 리뷰 코멘트와 함께 Teammate에게 수정 요청 |
...
**목표:** Leader가 Teammate을 생성하여 실제 코드 수정 + MR/PR 생성 + 검증까지 수행.
...
| **MCP** | Linear MCP, Git provider MCP/CLI | 이슈 트래킹과 코드 관리를 에이전트 도구로 노출. GitLab 정본은 `glab`, GitHub 잔존 리포는 `gh` 사용. |
...
### 7.5. Leader의 자동 MR/PR 리뷰

인수 조건 검증 이후 Leader가 MR/PR을 직접 리뷰하는 이유:
- Teammate은 자신이 작성한 코드의 문제를 스스로 발견하기 어려움 (자기 검증의 한계).
- Leader는 전체 시스템 맥락을 가지고 있으므로, 개별 MR/PR이 다른 서비스에 미치는 영향을 판단할 수 있음.
- 사람의 최종 리뷰 전에 자동화된 1차 리뷰로 품질을 높이고, 사람의 리뷰 부담을 줄임.
- MR/PR 리뷰 코멘트가 코드 호스트에 기록되어 변경 이력의 일부로 남음.
...
| 리스크 | 영향 | 완화 방안 |
|--------|------|-----------|
| Agent Teams 실험적 기능 불안정 | 세션 중단, 작업 유실 | Linear에 중간 결과를 지속적으로 기록. 각 Teammate의 MR/PR은 독립적이므로 부분 성과 보존. |
| LLM 환각으로 인한 잘못된 코드 수정 | 서비스 장애 | 인수 조건 검증 + Leader MR/PR 리뷰 + 사람의 최종 리뷰로 3중 안전망. |
...
| Leader MR/PR 리뷰 품질 | 잘못된 코드 통과 | Leader 리뷰는 1차 필터. 사람의 최종 리뷰를 생략하지 않음. |
...
# 4. MCP 서버 설정
# .claude/settings.json에 Linear MCP, Git provider MCP/CLI 설정
...
- **자동 롤백:** 배포 후 모니터링 메트릭 이상 시 자동 revert MR/PR 생성.
- **학습 루프:** 과거 성공/실패 패턴을 메모리에 저장하여 계획 수립 정확도 향상.
- **Slack/Discord 알림:** 실행 상태를 팀 채널에 실시간 보고.
- **배포 파이프라인 연동:** MR/PR 머지 후 자동 배포 트리거 (ArgoCD, GitLab CI/CD, GitHub Actions).
