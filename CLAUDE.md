...
| 검증·리뷰 통과(PR 오픈) | → **In Review** | 인수 조건 검증 + PR 리뷰 `APPROVE` 가 완료된 하위 이슈 |
| PR 머지 확인 | → **Done** | 머지가 실제로 확인되었거나 사용자가 완료를 확인한 티켓 |
...
- **Done 전환 조건**: PR이 **실제 머지**된 것을 확인했거나 사용자가 완료를 확인한 경우에만 `Done`으로 바꾼다. PR이 열려만 있고 아직 머지 전이면 `In Review`에 둔다.
...
- **리뷰를 `APPROVE`로 통과시키면 해당 하위 이슈 상태를 `In Review`로 변경한다.** (PR 오픈·머지 대기 상태)
...
- Linear 메인 티켓에 결과 코멘트 작성:
  ...
  - 리뷰 상태
- **Linear 티켓 상태 정리 (필수)**:
  - 각 하위 이슈: PR이 **머지된 것을 확인했으면 `Done`**, 아직 머지 대기 중이면 `In Review`로 둔다.
...
11. **Linear 티켓 상태를 작업 진행에 맞춰 항상 갱신한다.** 처리 착수 시 `In Progress`, 인수 조건·PR 리뷰 통과 시 `In Review`, **PR 머지가 확인되면 `Done`** 으로 전환한다. 어떤 작업이든(전체 Workflow·소규모·직접 수정 불문) 세션을 종료하기 전 관여한 모든 티켓(메인+하위)이 `Todo`/`Backlog`/`In Progress` 같은 중간 상태로 방치되지 않았는지 점검하고, **업무가 완전히 종료되면 관련 티켓을 반드시 `Done`으로 전환**한다. (「Linear 티켓 상태 관리」 섹션 참조)
...
mydata-mgmts-api (구 bomapp_my_data; SB 3.4/Java 21, 마이데이터 표준 mgmts 수신, 규제필수·PROD 미컷오버)
```
