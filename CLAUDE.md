...
- **반드시 각 Teammate은 독립된 git worktree에서 작업하도록 지시한다.** 동일 리포지토리에 여러 Teammate이 동시에 작업할 경우 코드 충돌을 방지하기 위함이다.
  - **⚠️ 절대 금지: overmind(Leader cwd) 리포에 다른 서비스 리포를 리모트로 추가하거나 그 리포를 fetch하지 않는다.** overmind 리포의 리모트는 영구히 `origin`(GitHub)·`gitlab`(정본) **둘뿐**이어야 한다. base 브랜치를 가져오겠다고 overmind 안에서 `git remote add <아무이름> <서비스>.git && git fetch` 하는 방식은 **금지** — 타깃 리포의 브랜치/태그 전체가 overmind 오브젝트 DB로 빨려 들어와 깃 그래프가 오염되고 `.git`이 수십~수백 MB로 부푼다. (실제 사고: next-backend·next-frontend가 overmind 그래프에 합쳐져 보임 → 리모트·태그 삭제 후 gc로 복구.)
  - worktree는 **반드시 타깃 서비스의 별도 클론(`services.yaml`의 `path`, overmind의 형제 디렉토리) 안에서, 그 리포 자신의 `origin` 기준으로** 만든다. overmind의 cwd에서 만들지 않는다.
  - Teammate 프롬프트에 작업 시작 시 다음을 수행하도록 명시한다:
    ```bash
    # 1) 타깃 서비스의 별도 클론으로 이동 (여기는 overmind 가 아니다!)
    cd <services.yaml의 path>            # 예: ../next-frontend  (overmind 의 형제 디렉토리)

    # 2) 안전 가드: 지금 위치가 '타깃 리포'가 맞는지 반드시 확인한다.
    #    origin 이 타깃 서비스 URL 이어야 한다. overmind URL 이거나 cd 가 실패했다면 즉시 중단.
    git remote get-url origin           # → 타깃 서비스(예: .../next-frontend) 여야 함
    #    클론이 없어서 cd 가 실패하면 → overmind 에 fetch 하지 말고, 별도 위치에 새로 clone 한다:
    #      git clone <services.yaml의 repo_url> <path> && cd <path>

    # 3) 타깃 리포 자신의 origin 기준으로 worktree 생성 (overmind 와 무관)
    git fetch origin
  ...
  - **`<feature_base_branch>`는 `services.yaml`의 `feature_base_branch` 필드 값을 사용한다.**
    ...
    - `infra`, `bomapp-vkey`, `next-frontend`, `bomapp-backoffice`: `main`
    - `mydata-agent`: `prod`
...
- Teammate 종료 및 팀 정리.
- **Teammate이 사용한 worktree 정리**: 각 worktree에 대해 `git worktree remove <path>` 실행. 작업 브랜치는 MR/PR 머지 전까지 유지한다. worktree는 **타깃 서비스 클론**에 속하므로 해당 클론 디렉토리에서 제거한다(overmind 가 아니다).
- **🧹 overmind 리포 리모트 오염 점검 (필수)**: `git remote -v` 로 overmind의 리모트가 `origin`(GitHub)·`gitlab`(정본) **둘뿐**인지 확인한다. 다른 서비스 리포를 가리키는 군더더기 리모트(예: 과거 `glab-base`)가 보이면 `git remote remove <name>` → 딸려온 외부 태그가 있으면 `git tag -d <외부태그>` → `git gc --prune=now` 로 정리한다. (배경: 절대 규칙 12 — 타깃 리포를 overmind에 fetch하면 그 브랜치·태그가 깃 그래프에 섞여 들어옴.)
...
12. **overmind 리포의 리모트를 오염시키지 않는다.** overmind(Leader cwd) 리포에는 **다른 서비스 리포를 리모트로 추가하거나 fetch하지 않는다.** overmind의 리모트는 영구히 `origin`(GitHub)·`gitlab`(정본) **둘뿐**이다. 다른 서비스 작업은 그 서비스의 **별도 클론(`services.yaml`의 `path`)** 안에서 수행하며, base 브랜치도 그 클론 자신의 `origin`에서 가져온다. overmind 안에서 `git remote add <서비스>.git`/`git fetch <서비스>` 를 실행하면 타깃 리포의 브랜치·태그 전체가 overmind 그래프로 섞여 들어와 오염된다(next-backend·next-frontend 사고). 세션 종료 전 `git remote -v` 로 overmind 리모트가 `origin`·`gitlab` 둘뿐인지 확인하고, 군더더기 리모트가 보이면 `git remote remove` + `git tag -d`(딸려온 외부 태그) + `git gc --prune=now` 로 즉시 정리한다. (Finalize 단계 참조)
...
### Source Control
- **기본 정본은 사내 GitLab** `gitlab.bomapp.co.kr/bomapp/*` 이다. `infra`, `next-backend`, `next-frontend`, `az-was`, `apps-distribution`, 네이티브 앱, `backoffice` 작업은 GitLab MR(`glab`)로 처리한다.
...
bomapp-backoffice (Bun/Turborepo / Next.js + NestJS)
  ├─ apps/frontend — 내부 백오피스 UI (S3+CloudFront)
