...
- **색 대비(WCAG AA)를 빌드 시 확보**(리뷰 상습 결함 선제 차단): 채도 높은 상태색 warn `#FF8A00`·safe `#12B886`은 **막대·아이콘 등 비텍스트 그래픽에만** 쓰고, **텍스트·수치·상태 라벨엔 darker 변형**(오렌지→`#C2410C`대, 그린→`#0E7C5A`대, 밝은 배경서 ≥4.5:1)을 쓴다. 회색 캡션은 `#8A94A6`(흰/연배경 ~2.8~3:1 미달) 대신 본문급은 `#3B4252`(ink2). 9px 같은 초소형 텍스트 금지(≥11~12). 상태는 **색 + 텍스트 라벨 이중 인코딩**. (검수 기준은 `design-review` 스킬.)
  - **⚠️ 브랜드 primary `#3c7ae5`도 흰 배경서 4.11:1 → AA 본문 미달.** 인라인 링크·eyebrow·작은 라벨(<18px)엔 **`#2f65cc`(primary/hover, 5.45:1)**, 막대 축 회색은 `#8a8f99`(3:1) 말고 **`#5b6270`(5.7:1)**, 빨강 수치는 `#d12d2d`(5:1)·칩 주황 `#c2410c`/보라 `#7a35d1`/핑크 `#b03073`(모두 AA)을 쓴다. `#3c7ae5` 원색은 큰 Bold(≥18px=AA large 3:1)·그래픽에만. (TA-147 리뷰 지적 사례.)
- **카드/컨테이너 내부 패딩**(16~20px 일관): plain frame에 자식을 **절대배치**하면 clip이 안 되므로 **프레임 height ≥ 마지막 자식의 `y+height` + 하단패딩(≈18)**을 반드시 보장한다(안 그러면 인사이트 박스 등이 카드 밖으로 삐져나옴 — v4 비교카드 실수). 콘텐츠가 가변이면 카드 자체를 `createAutoLayout` + `padding`으로 만들어 자동으로 hug되게 하는 게 안전.
  - **⚠️ 단, auto-layout 카드라도 `resize(w,h)`를 부르면 *높이축* 사이징모드가 FIXED로 되돌아가 hug가 깨진다** → 콘텐츠 짧으면 **하단 데드스페이스**(상·하 패딩 비대칭), 길면 **CTA/콘텐츠 오버플로우·클리핑·겹침**. **높이축은 레이아웃 방향에 따라 다르다** — 세로(VERTICAL)=`primaryAxisSizingMode`, **가로(HORIZONTAL)=`counterAxisSizingMode`**. 콘텐츠 카드·**리스트 행**은 폭만 `resize`/`layoutSizingHorizontal`로 잡고 **높이축은 항상 `'AUTO'`(hug)** 로 두되, resize를 썼다면 **그 직후 높이축 사이징모드를 `'AUTO'`로 다시 set** 한다. (TA-147: BillboardCard·CategoryCard 하단 데드스페이스, AnalysisCard·옵션카드 클리핑[세로축]; **InsuranceRow는 *가로* 행인데 counterAxis FIXED h80 → 칩 2줄 래핑이 넘쳐 점검배지와 겹침**[가로축] — 리뷰도 처음엔 세로축만 봐서 놓침.)
  - **⚠️ 래핑 태그/칩 row는 full-width 줄을 줘라**: 커버리지 칩·뱃지처럼 가변 개수 태그는 **좁은 컬럼(가격·값 옆 mid 컬럼)에 끼우면** 2~3개만 돼도 2줄로 래핑돼 넘친다 → 태그 row를 **이름 아래 full-width(필요 시 logo폭만큼 들여쓰기)** 로 분리해 한 줄에 들이고, 부모 행은 높이 hug로 둔다(많으면 깔끔히 래핑). (TA-147 InsuranceRow를 가로행→세로[topRow + full-width chips]로 재구성해 해결.)
  - **빌드 자가검수**(스크린샷 전): 각 콘텐츠 카드/행의 **높이축 사이징모드가 AUTO**인지(고정요소 제외) + `topPad≈bottomPad`(±6px)인지 + **오버플로우 좌표 sweep**(`자식.x+w>부모.w` 또는 `자식.y+h>부모.h`, 클립-peek 제외)을 1회 돌려 0건인지 확인. 인스턴스가 폭만큼 안 차거나 세로로 비면 마스터 hug부터 점검.
...
- **⚠️ `node.query()` CSS 셀렉터는 한글 레이어명 파싱 실패**(`Invalid selector: unexpected character (0xeb)`) → 한글 이름 노드는 `findOne/findAll(n=>n.name===...)` 술어로 찾는다(영문 이름엔 query OK). **⚠️ `resize()`가 auto-layout 카드의 *높이축* 사이징모드를 FIXED로 리셋**(세로 레이아웃=primaryAxis, 가로 레이아웃=counterAxis) → hug 프레임은 자식을 다 채운 뒤 **높이축 사이징모드를 `'AUTO'`로 resize 이후 다시 설정**(안 하면 데드스페이스/오버플로우/겹침; 위 카드 패딩 항목 참조). **아이콘 변형은 INSTANCE_SWAP 프로퍼티**(작은 아이콘 컴포넌트 N개 + `addComponentProperty('icon','INSTANCE_SWAP',compId)`)—인스턴스는 자식 추가 불가라 슬롯 변형의 정석.
